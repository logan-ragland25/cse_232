...
int main() {
    ...
    std::cout << (!e.AddOrder({"Nahum", "Sell", "BTC", 50, 1000}));
    // Can't place order because Nahum doesn't have enough BTC
    std::cout << (!e.AddOrder({"Nahum", "Buy", "BTC", 50, 200}));
    // Can't place order because Nahum doesn't have enough USD
    std::cout << (e.AddOrder({"Nahum", "Sell", "BTC", 4, 1000}));
    std::cout << (e.AddOrder({"Nahum", "Buy", "ETH", 3, 2}));
    std::cout << (e.AddOrder({"Dolson", "Buy", "BTC", 7, 1}));
    std::cout << (e.AddOrder({"Ofria", "Sell", "ETH", 2, 500}));
